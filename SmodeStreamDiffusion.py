...
class CommandType(enum.Enum):
    START = 1  # Sent by server: client should start computing a new frame
    STOP = 2  # Sent by server: client should exit
    CONFIG = 3  # Sent by server: update configuration; payload includes:
    FINISHED = 4  # Sent by client: signals frame computation is done; payload is the CUDA IPC info
...
class Packet:
    ...
    def to_bytes(self) -> bytes:
        ...
        payload_bytes = struct.pack(
            ENDIAN_FORMAT + UINT32, self.cmd.value) + self.payload
        size = len(payload_bytes)
        header = struct.pack(ENDIAN_FORMAT + UINT32 +
...
class FrameDataPacket(Packet):
    def __init__(self, cmd: CommandType, device: int, handle: bytes, event_handle: bytes, storage_size_bytes: int, storage_offset_bytes: int, channels: int, w: int, h: int):
        ...
        payload += struct.pack(ENDIAN_FORMAT + UINT32,
                               len(event_handle)) + event_handle
        payload += struct.pack(ENDIAN_FORMAT + UINT32 + UINT32 + UINT32 + UINT32 +
                               UINT32, storage_size_bytes, storage_offset_bytes, channels, w, h)
...
class ConfigPacket(Packet):
    ...
    def from_bytes(self, data: bytes):
        ...
        offset += 8
        t_index_list_len = 0
        self.width, self.height, t_index_list_len = struct.unpack_from(ENDIAN_FORMAT + UINT32 + UINT32 + UINT32, data, offset)
        offset += 12
        for i in range(t_index_list_len):
            if offset + 4 > len(data):
                raise ValueError("Insufficient data for t_index_list in CONFIG")
            t_index_list, = struct.unpack_from(ENDIAN_FORMAT + UINT32, data, offset)
...
class StreamDiffusionSmodeTexture:
    def __init__(self, device: int, width: int, height: int, channels: int, dtype: torch.dtype):
        ...
        self.stream_diffusion_tensor = torch.empty((self.height, self.width, channels), dtype=self.dtype, device=self.device)
        self.smode_tensor = torch.empty((self.height, self.width, channels), dtype=torch.float32, device=self.device)
    ...
    def copy_smode_to_stream_diffusion(self):
        ...
        """
        # Convert smode tensor to the stream tensor's dtype and device, then copy
...
def is_socket_connected(sock):
    try:
        # Try to peek 1 byte without consuming it.
        data = sock.recv(1, socket.MSG_PEEK)
        # If recv returns 0 bytes, the connection is closed.
        return len(data) > 0
    except BlockingIOError:
        # No data available right now but the socket is still connected.
        return True
    except socket.error:
        # Any other socket error implies the connection is likely broken.
        return False

class App:
    def __init__(self, config: Args, device: torch.device, torch_dtype: torch.dtype):
        self.config = config
        self.device = device
    ...
    def _create_tensors(self, channels, w, h):
        self.input_tensors = StreamDiffusionSmodeTexture(self.device, w, h, channels, self.torch_dtype)
        self.output_tensors = StreamDiffusionSmodeTexture(self.device, w, h, channels, self.torch_dtype)
        (
            tensor_type,
            tensor_size,
            tensor_stride,
            ...
            event_handle,
            event_sync_required,
        ) = self.input_tensors.smode_tensor_ipc_info
        packet = FrameDataPacket(CommandType.INPUT, device, handle[2:], event_handle, storage_size_bytes, storage_offset_bytes, 3, self.width, self.height)
        ...
        try:
            while True:
                ...
                ready_to_read, _, in_error = select.select(
                    [self.socket], [], [], 0.1)
                if ready_to_read:
                    # Attempt to read all available messages
                    while True:
                        try:
                            cmd, payload = recv_message(self.socket)
                            if cmd is None:
                                break
                            # Keep only the latest message per command
                            if CommandType.START in messages:
                                logging.warning(
                                    "Received multiple START commands; only the latest will be processed")
                            messages[cmd] = payload
                        except socket.error:
                            # No more data available; break out of the inner loop
                            break
                if in_error:
                    ...
                    if cmd == CommandType.START:
                        ...
                        if self.mode == Mode.IMAGE_TO_IMAGE:
                            if self.input_tensors.stream_diffusion_tensor is not None:
                                permuted_input_texture = F.vflip(self.input_tensors.stream_diffusion_tensor.permute(2, 0, 1))
                            x_output = self.stream.img2img(image=permuted_input_texture)
                        ...
                        (
                            ...
                            event_handle,
                            event_sync_required,
                        ) = self.output_tensors.smode_tensor_ipc_info
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device", type=int, required=True, help="The cuda device index to use")
    parser.add_argument("--model", type=str, required=True, help="Model name to use")
    args = parser.parse_args()
    config = Args(port=args.port, uuid=args.uuid, width=args.width, height=args.height, device=args.device, model=args.model)

    if not torch.cuda.is_available():
        logging.error("CUDA is not available")
        exit("CUDA is not available")

    # check if cuda device is available config.device
    if config.device < 0 and config.device >= torch.cuda.device_count():
